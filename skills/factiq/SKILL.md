...
`source_link.source_url` immediately beside the claim as a Markdown link. Do
not make a second tool call solely to find a citation, reconstruct a URL, or
substitute a related press release. `source_precision="document"` is not exact
context. If `source_url` is null, state that the direct link is unavailable and
use the returned locator/document ID only as provenance metadata.
...
`sec_guidance` targets, and filed `sec` actuals as separate source classes. Put
the row's provided source URL beside every quote you use; if it is null, say
the link is unavailable instead of searching for or inventing one.
